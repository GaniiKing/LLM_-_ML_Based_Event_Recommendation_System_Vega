from flask import Flask, request, jsonify
from openai import OpenAI
import joblib
import pandas as pd
import numpy as np
import ast
import json

app = Flask(__name__)

# === MODEL LOADING ===
client = OpenAI(
    api_key="cf345e1d-4f4e-46ee-b1de-ccd400e505ee",
    base_url="https://api.sambanova.ai/v1",
)

model_components = joblib.load('service_recommender.pkl')
clf = model_components['model']
encoders = model_components['encoders']
tfidf_instructions = model_components['tfidf_instructions']
mlb = model_components['mlb']

# === UTILITY FUNCTIONS ===
def check_null_values(data):
    return [key for key, value in data.items() if value is None]

def get_event_recommendations(query):
    input_data = f"""
    You are an assistant helping extract structured data for an event planner.Youre name is VEGA AI working for VEGASHOW 
    They just said:
    "{query}"
    If user say anything out of the event context please return none
    Please update the appropriate field based on the user's latest message, and return the full JSON with these keys:
    - event
    - vibe
    - date
    - budget
    - time
    - duration
    - venue
    - location
    - guests
    - special_instructions
    make sure you get the Date in this format DD-MM-YYYY and time in 24hrs format and remember the present year is 2025
    and for duration just get the number of hours only the numerical value
    Set any fields not yet provided to null.
    i dont need any code or explanations for updating or altering i just need only the json response as i desired
    if any null values are present, then ask the user to provide the missing information
    gimme all the data only in lower case letters
    Return only the updated JSON object. No explanation. donot gimme `` these also
    """

    response = client.chat.completions.create(
        model="Llama-4-Maverick-17B-128E-Instruct",
        messages=[{"role": "user", "content": [{"type": "text", "text": input_data}]}],
        temperature=0.1,
        top_p=0.1
    )
    content = response.choices[0].message.content
    return content

def get_package_recommendations(input_data):
    events = pd.read_csv('vega_detailed_csv_files/generated_events.csv')
    events_i = events[events['event_type'] == input_data['event']]
    if len(events_i) == 0:
        return None, None
    flexible_budget = float(input_data['budget']) * 1.1
    budget_filtered_events = events_i[events_i['cost'] <= flexible_budget]
    if len(budget_filtered_events) == 0:
        return None, None
    selected_event = budget_filtered_events.sort_values('cost', ascending=False).iloc[0]
    selected_event['services_included'] = ast.literal_eval(selected_event['services_included'])
    return selected_event['package_id'], selected_event.to_dict()

def get_service_recommendations(input_data):
    input_df = pd.DataFrame([input_data])
    input_df['event_id'] = encoders['event_id'].transform(input_df['event'])
    input_df['vibe'] = encoders['vibe'].transform(input_df['vibe'])
    input_df['location'] = encoders['location'].transform(input_df['location'])
    input_df['venue'] = encoders['venue'].transform(input_df['venue'])

    instructions_tfidf = tfidf_instructions.transform(input_df['special_instructions'].astype(str))

    X_input = np.hstack([
        input_df[['event_id', 'vibe', 'location', 'budget', 'venue', 'duration', 'guests']].values,
        instructions_tfidf.toarray()
    ])

    predictions = clf.predict(X_input)
    prediction_probs = clf.predict_proba(X_input)

    confidence_scores = []
    for i, service in enumerate(mlb.classes_):
        if service.lower() in ['no', 'n', 'o'] or len(service) <= 1:
            continue
        service_probs = prediction_probs[i]
        confidence = service_probs[0][1] if isinstance(service_probs, np.ndarray) and len(service_probs.shape) > 1 else (
            service_probs[1] if len(service_probs) > 1 else service_probs[0])
        if confidence > 0.1:
            confidence_scores.append((service, confidence))

    sorted_services = sorted(confidence_scores, key=lambda x: x[1], reverse=True)[:5]
    services_df = pd.read_csv('vega_detailed_csv_files/generated_providers.csv')

    recommendations = []
    for service, confidence in sorted_services:
        match = services_df[services_df['service_id'] == service]
        if not match.empty:
            recommendations.append({
                'service_id': service,
                'confidence': confidence,
                'category': match['service_category'].values[0],
                'title': match['service_title'].values[0],
                'description': match['service_description'].values[0]
            })

    return recommendations

# === ROUTE ===
@app.route('/recommend', methods=['POST'])
def recommend():
    req = request.json
    user_query = req.get("query", "")
    try:
        result = get_event_recommendations(user_query)
        json_data = json.loads(result)
    except Exception as e:
        return jsonify({"error": "Failed to extract event details", "raw_response": result, "error_message": str(e)}), 400

    null_fields = check_null_values(json_data)
    if null_fields:
        return jsonify({"missing_fields": null_fields, "event_data": json_data}), 200

    services = get_service_recommendations(json_data)
    package_id, package_details = get_package_recommendations(json_data)

    return jsonify({
        "event_data": json_data,
        "service_recommendations": services,
        "package_id": package_id,
        "package_details": package_details
    }), 200

# === ENTRY POINT ===
if __name__ == "__main__":
    app.run(debug=True)
